...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .share-card{width:720px;padding:28px;background:#fff;color:#111;font-family:"Microsoft JhengHei","Noto Sans TC",system-ui,sans-serif}
    .share-card h1{text-align:center;margin:0 0 16px;font-size:30px}
    .share-card p{margin:8px 0;font-size:18px}
    .share-card table{width:100%;border-collapse:collapse;margin-top:16px;font-size:18px}
    .share-card th,.share-card td{border:1px solid #333;padding:10px;text-align:left}
    .share-card th{background:#eee}
    .share-card td:nth-child(2),.share-card td:nth-child(3),.share-card td:nth-child(4),.share-card th:nth-child(2),.share-card th:nth-child(3),.share-card th:nth-child(4){text-align:right}
    .share-card .share-total{text-align:right;font-size:24px;font-weight:900;margin-top:14px}
    @media print{
      body{background:#fff}
      ...
      .print-total{font-size:20px;font-weight:bold;text-align:right;margin-top:12px}
    }
    @media(max-width:760px){.topbar{padding:12px}.brand img{width:48px;height:48px}.brand strong{font-size:19px}.wrap{padding-bottom:126px}.customer-box,.search-row,.items,.bottom-inner,.metrics,.inventory-row,.custom-grid{grid-template-columns:1fr}.search-row{top:65px}.main-tabs{grid-template-columns:1fr}.item{grid-template-columns:76px minmax(0,1fr);padding:9px}.item img{width:76px;height:76px}.item h3{font-size:16px}.qty{grid-template-columns:40px minmax(46px,1fr) 40px}.qty button{width:40px;height:40px}.checkout{width:100%}.admin-tabs{grid-template-columns:repeat(2,minmax(0,1fr))}.modal-body{padding:12px}.line{grid-template-columns:1fr}.line-actions{justify-content:flex-start}.return-line{grid-template-columns:24px minmax(0,1fr)}}
  </style>
</head>
<body>
  <header class="topbar">
  ...
  <script>
    ...
    function renderReceipt(){
      pendingOrder = { name:document.getElementById("customerName").value.trim(), note:document.getElementById("customerNote").value.trim(), items:cartItems() };
      drawReceipt();
    }
    function drawReceipt(){
      ...
      document.getElementById("receiptContent").innerHTML=`
        <div class="screen-receipt">
          <p><strong>大名：</strong>${pendingOrder.name}</p>${pendingOrder.note?`<p><strong>備註：</strong>${pendingOrder.note}</p>`:""}
          <div class="receipt-list">${pendingOrder.items.map((item,index)=>`<div class="line"><span>${itemName(item)}<br><small>單價 ${money(itemPrice(item))} × ${item.qty} = ${money(itemSubtotal(item))}</small></span><span class="line-actions"><button data-receipt-dec="${index}">−</button><button data-receipt-inc="${index}">＋</button><button class="remove" data-receipt-remove="${index}">刪除</button></span></div>`).join("") || `<div class="empty">訂單已沒有品項。</div>`}</div>
        ...
        <div class="print-receipt" style="display:none">
          ...
          <div class="print-total">總計：${money(total)}</div>
        </div>`;
    }
    function buildShareCard(){
      ...
      card.innerHTML=`
        ...
        <div class="share-total">總計：${money(total)}</div>`;
    ...
    function orderCard(order){
      return `<div class="order-card"><div class="order-title"><span>${order.id}</span><span class="badge ${order.status}">${statusText(order.status)}</span></div><div class="meta">送單 ${order.createdAt || "-"}${order.shippedAt?` · 出貨 ${order.shippedAt}`:""}${order.returnedAt?` · 退貨 ${order.returnedAt}`:""}</div><div class="receipt-list">${order.items.map(i=>`<div class="line"><span>${itemName(i)} · 單價 ${money(itemPrice(i))} × ${i.qty}</span><strong>${money(itemSubtotal(i))}</strong></div>`).join("")}</div><div><strong>總計：</strong>${money(orderTotal(order)-returnedTotal(order))}${order.note?` ｜備註：${order.note}`:""}</div></div>`;
    }
    ...
    document.getElementById("saveOrder").addEventListener("click",()=>{
      ...
      const order={ id:makeOrderId(pendingOrder.name), name:pendingOrder.name, note:pendingOrder.note, items:pendingOrder.items.map(i=>({...i,picked:false})), status:"new", returns:[], createdAt:nowText(), createdAtISO:new Date().toISOString() };
