...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    @media(max-width:760px){.topbar{padding:12px;align-items:flex-start}.brand img{width:48px;height:48px}.brand strong{font-size:19px}.top-actions{display:grid;grid-template-columns:1fr;min-width:92px}.top-actions button{width:100%;padding:0 8px}.wrap{padding-bottom:126px}.customer-box,.search-row,.items,.bottom-inner,.metrics,.inventory-row,.custom-grid{grid-template-columns:1fr}.search-row{top:65px}.main-tabs{grid-template-columns:1fr}.item{grid-template-columns:76px minmax(0,1fr);padding:9px}.item img{width:76px;height:76px}.item h3{font-size:16px}.qty{grid-template-columns:40px minmax(46px,1fr) 40px}.qty button{width:40px;height:40px}.checkout{width:100%}.admin-tabs{grid-template-columns:repeat(2,minmax(0,1fr))}.modal-body{padding:12px}.line{grid-template-columns:1fr}.line-actions{justify-content:flex-start}.return-line{grid-template-columns:24px minmax(0,1fr)}}
  </style>
</head>
<body>
  <header class="topbar">
    <div class="brand"><img src="assets/logo.jpg" alt="福緣素食 logo"><div><strong>福緣訂單系統</strong><span>輸入大名，選品項，確認後送出</span></div></div>
    <div class="top-actions"><button class="secondary" id="openExternal">瀏覽器開啟</button><button class="admin-open" id="openAdmin">後台</button></div>
  </header>
  ...
  <script>
    ...
    function drawShareCanvas(){
      ...
      ctx.textAlign="right";
      ctx.fillText(`總計：${money(total)}`,width-padding,y+28);
      return canvas;
    }
    ...
    async function openExternalBrowser(){
      ...
      try{
        ...
        alert("已嘗試開啟新瀏覽器分頁，並複製網址。若仍在 LINE 內建瀏覽器，請用右上角選單選「在瀏覽器開啟」。");
      }catch(error){
        alert(`請複製網址後用瀏覽器開啟：${url}`);
      }
    }
    ...
    document.getElementById("openExternal").addEventListener("click",openExternalBrowser);
